...
function build_omniROM_twrp() {
    ...
    cp -rf ${build_cache_customROM:?}/target/product/"${1:?}"/boot.img ${build_cache_SODP_TWRP:?}/"${1:?}"/
    echo "####$1 END####"

    case "${1:?}" in
        "akari")
            echo "####Aurora START####"
            cp -rf ${build_cache_customROM:?}/target/product/"${1:?}"/boot.img ${build_cache_SODP_TWRP:?}/aurora/
            echo "####Aurora END####"
        ;;
    esac
...
function build_stockROM_twrp() {
    ...
    # Unzip twrp boot img
    cp -rf ${build_cache_SODP_TWRP:?}/"${1:?}"/${boot_img_name:?} ${current_dir_tools_aik:?}/
    bash ${current_dir_tools_aik:?}/unpackimg.sh --nosudo
    # copy the stock fstab
    cp -rf "${current_dir:?}"/etc/* ${current_dir_tools_aik:?}/ramdisk/etc

    rm ${current_dir_tools_aik:?}/${boot_img_name:?}

    # copy the self compiled stock kernel with merged dtbo
    cp -rf ${build_cache_stock_kernel:?}/"${1:?}"/${boot_img_kernel_name:?} ${current_dir_tools_aik:?}/split_img/${boot_img_kernel_name:?}
    ...
    cp -rf ${current_dir_tools_aik:?}/${boot_img_new_name:?} ${build_out:?}/"${1:?}"/twrp-"${1:?}".img
    cp -rf ${current_dir_tools_aik:?}/${boot_img_new_name:?} ${current_dir_tools_aik:?}/${boot_img_name:?}
